...
Total_Distance_Initial = OB.Complete_Distance_Not_Random(rnd_sol_1, Dist_Dataframe) # Get the distance of the initial solution
...
for i in range(Runs):
    ...
    ### Step 1: Produce the 2-Opt Swap Neighbourhood ###
    ...
    for i2 in List_of_N: # For i2 in every combintation of 2's
        X_Swap = []
    ...
    ### Step 2: Calculate the OF value all items in the 2_Opt Swap neighbourhood ###
    
    for i3 in Store_all_Combinations:
        
        Cost = OB.Complete_Distance_Not_Random(i3,Dist_Dataframe) # The distance function
        
        Penalty_1 = OB.Penalty_1(i3,20,Cap_Dataframe) # The penalty function for carrying more than 100
        Penalty_2 = OB.Penalty_2(i3,200,300) # The penalty of warehouses' locations
        Penalty_3 = OB.Penalty_3(i3,400) # The penalty of warehouses' locations
               
        Total_Distance_N_i = Cost + Penalty_1 + Penalty_2 + Penalty_3 # Add the costs to the penalties
        
        i3 = i3[np.newaxis] # Change axis to horizontal
        
        OF_Values_for_N_i = np.column_stack((Total_Distance_N_i,i3)) # Put the distance next to its solution
    ...
    ### Step 3: Get best so far solution in the "Save Solutions Here" ###
    ...
    ### Step 4: Check solution against Tabu list ###
    ...
    # Step 4: Set some conditions in order to "kick-start" the search when stuck in a local optimum
    ...
    # Step 5: Return to best so far in case you kept getting worse solutions
    
    if Mod_Iterations_3 == 0:
...
Without_Penalty_Cost = OB.Complete_Distance_Not_Random(Without_Penalty[0],Dist_Dataframe)
